...
%% ----------------- PART 6: PERTURBATION OF THE SYSTEM -------------------
% Perturbation 1
u = b * c * ones (1,2);
...
for var2 = 4
    for var3 = 3
        ...
        for var1=19
            ...
            Istim(:,250/dt+1:end) = 15; % add perturbation
            ...
            title(['Two-Neuron Oscillating Network - Perturbation of I_{stim}=15 mA at 250 ms'])
            setfont; axis tight;
            ...
            setfont; axis tight;
            print -dpng bme515_hw1_part6_1.png
            
            %             fprintf(sprintf('%g %g %g \n',var1, var2, var3))
            %             keyboard
            %% -------------- QUANTIFICATION OF THE OSCILLATION ---------------
            ...
            timeloc = [1:min(length(period1),length(period2))];
...
for var2 = 4
    for var3 = 3
        ...
        for var1=19
            ...
            Istim(1,5/dt+1:end) = 8;
            Istim(2,(5+var1)/dt+1:end) = 8;
            Istim(:,250/dt+1:end) = 2; % negative perturbation
            Istim(:,(tstop-25)/dt+1:end) = 0;
            ...
            %         vline(122)
            xlabel('Time (ms)'); ylabel('v(t)')
            title(['Two-Neuron Oscillating Network - Perturbation of I_{stim}=2 mA at 250 ms'])
            setfont; axis tight;
            ...
            legend('Neuron 1','Neuron 2')
            setfont; axis tight;
            print -dpng bme515_hw1_part6_2.png
            ...
            timeloc = [1:min(length(period1),length(period2))];
            for k = 1:length(timeloc)
                timeDifference(k) = [locs1(k) - locs2(k)] * dt * 1e-3;
            end
